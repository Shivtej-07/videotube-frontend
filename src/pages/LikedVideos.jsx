...
function LikedVideos() {
    ...
    useEffect(() => {
        const fetchLikedVideos = async () => {
            try {
                ...
                // The API returns an array of Like objects which contain the video in the 'video' field
                const likedVideosList = response.data.data.map(item => item.video).filter(video => video !== null);
                setVideos(likedVideosList || []);
            } catch (err) {
                console.error("Failed to fetch liked videos:", err);
                setError("Could not load liked videos.");
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) return <div className="text-center mt-8">Loading liked videos...</div>;
    ...
    return (
        <div>
            ...
            {videos.length === 0 ? (
                <p className="text-center text-gray-400">No liked videos found.</p>
            ) : (
